...
#[wiring]
struct State<const N: usize>(Wire, Id<N>, [Addr<N>; 3]);

type Cell<const N: usize> = ([Addr<N>; 3], [Addr<N>; 3]);

#[component]
pub fn cell<const N: usize>(&mut self, cl: Wire, p: (Wire, Wire), (msg_out, _msg_in): Cell<N>) {
  let state = self.wiring();
  self.register::<State<N>>(cl, state);
  let state = state.val;
  let msg_out_p01 = {
